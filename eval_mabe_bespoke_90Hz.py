...
def run_encode(
    ...
    # -- load, reindex, slice submission rows once --------------------------
    ...
    for analysis_key in keys:
        ...
        for combine in ["bams"]:
            ...
            # arr = np.array(submission, dtype=object)
            ...
            with open(sub_path, 'wb') as f:
                npy_format.write_array_header_2_0(
                    f, npy_format.header_data_from_array_1_0(arr)
                )
...
'''
python eval_mabe_bespoke.py \
  --data_path /hpc/home/yw789/tdunn/befound_code/bams/data/mabe_90hz/ \
  --results_path /hpc/group/tdunn/yw789/befound_results/ \
  --out_dir submissions \
  --combine max \
  --batch_size 512 \
  --device cuda
