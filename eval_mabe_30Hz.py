"""
yw: Run befound 2D encoders on MABe 2022 Mouse Triplets and build submission files.

Pipeline:
    load_mice_triplet           -> (N*3, T, K, 2) keypoints
                                   split_mask (video-level), batch (per-mouse video idx)
    reindex (ABE_REINDEX)       -> (N*3, T, 12, 2) in befound keypoint order
    keep submission rows only   -> (N_sub*3, T, 12, 2)   (memory: skip train)
    for each model in key_dict:
        load model
        for each of the N_sub*3 mouse trajectories:
            pad front/back (edge, 25 frames each side)
            sliding window (W=51, stride=1)        -> (T, W, K, 2)
            mini-batch through prepare_batch_2d_bespoke(path A) + model.encode
            keep mu of the center frame for every t -> (T, d)
        format_submission (combine 3 mice along feature dim) -> .npy

- Windows for one trajectory are built on demand from a single edge-padded
  copy of that trajectory (via sliding_window_view, essentially a strided
  view), so peak host RAM for the window buffer is one (T+50, K, 2) array.
- We encode *only submission* trajectories by default; flip `encode_all=True`
  if you also want train embeddings on disk.
- z_all / z_sub and the GPU model are freed between key_dict entries.
- prepare_batch_2d_bespoke is the same function used at training time, in
  its "path A" branch (data["pose"] -> midfwd-aligned x3d/x2d). At inference
  we pass `NO_AUGMENT_2D` so single_ablation / 2d_td / kpt_shuffle are all
  off.
- The function emits both `x2d` (for ResVAE2D / CIResVAE2D) and `x3d` with
  z=0 (for ResVAE / CIResVAE under the 2d_td=1 regime), so the same call
  site works for all 6 models in KEY_DICT.
- Coordinates are normalized by OFFSETS_3D_MABE22_SUM (~242.1) -- the
  median-segment-length sum of the MABe22 12-keypoint mouse skeleton.
  This puts the inference data on the same unit-skeleton scale the
  encoders saw during training (which used `use_default_offsets=True` /
  OFFSETS_3D_SUM on the 4_mice 18-keypoint skeleton).
"""
...
'''
python eval_mabe_30Hz.py \
  --data_path /hpc/home/yw789/tdunn/befound_code/bams/data/mabe/ \
